...
fun! vam#install#BisectCompletion(A, L, P, ...)
  let list = ['vim','gvim']
  return list
endf
"}}}

" arbitrarely butting Bisect* stuff here so that it does not get loaded always {{{1
fun! vam#install#BisectStepBad(vim, plain, addons, vimrc_tmp) abort
...
" ... must be cache
fun! vam#install#BisectList(vim_executable, skip_initial, addons, force_addons, ...) abort
  let cache = a:0 > 0 ? a:1 : {}
  ...
  let test      = 'let bad = vam#install#BisectStepBad(a:vim_executable, 0, addons + a:force_addons, s:vimrc_tmp)'
  let testPlain = 'let bad = vam#install#BisectStepBad(a:vim_executable, 1, addons + a:force_addons, s:vimrc_tmp)'
  ...
  if len(addons) == 1
    ...
    let r = vam#install#BisectList(a:vim_executable, 1, [], a:force_addons, cache)
    if r.problem_found
      return r
    else
  ...
  if bad
    ...
    return vam#install#BisectList(a:vim_executable, 1, addons, a:force_addons, cache)
  else
    " right side should contain issue, verify it
    let addons = a:addons[len(a:addons)/2:-1]
    exec test
    if bad
      let r = vam#install#BisectList(a:vim_executable, 1, addons, a:force_addons, cache)
      if r.problem_found
        return r
      else
        throw "unexpected"
      ...
      " " fix left_half
      " let r = vam#install#Bisect(a:vim_executable, right_half, left_half)

      " " fix right_half
      " let r = vam#install#Bisect(a:vim_executable, left_half, right_half)
    endif
  endif
endf
